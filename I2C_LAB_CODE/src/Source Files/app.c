...
void scheduled_letimer0_comp1_cb(void){
  ...
  Si1133_read(SI1133_REG_READ_CB, I2C1);
  /*
  if(led_color == 0){
...
/***************************************************************************//**
 * @brief
 *   The si1133_test function was supposed to test our recieved value
 *   from the sensor. If the value was correct, we would make a green
 *   light go off on the LED. If it was incorrect, we would make a red light
 *   go off on the LED
 * @details
 *   Calls the Si1133_return_data() function to recieve the data from the
 *   sensor. The value we compare to is 51.
 *
 * @note
...
void si1133_test(){
  ...
  uint32_t data_value = Si1133_return_data();
  if(data_value == READVALUETOCOMPARETO){
      leds_enabled(RGB_LED_1, COLOR_GREEN, true);
  }
  else{
      leds_enabled(RGB_LED_1, COLOR_RED, true);
