 ...
 ******************************************************************************/
void i2c_start(I2C_TypeDef *i2c,uint32_t *result_address, uint32_t rw, uint32_t register_address,uint32_t callback_value){
  ...
  //Replaced with end.. right?
  //local_state->busy = false;
  //sleep_mode_unblock(I2C_EM_BLOCK);
}
/***************************************************************************//**
 * @brief
...
void i2c_ack(I2C_STATE_MACHINE *local_state){
  switch(local_state->current_state){
    ...
    case resending_data:
      ...
      break;
    case reading:   //do I need to do this?
      break;
    case end:
      EFM_ASSERT(false);
      break;
