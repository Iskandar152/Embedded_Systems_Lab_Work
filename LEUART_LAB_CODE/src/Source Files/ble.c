...
/***************************************************************************//**
 * @brief
 *   ble_open initializes a local leuart_open_struct so as to pass it to
 *   leuart_open
 * @details
 *   The baudrate, databits, enable, parity, and the tx/rx routing information
 *   are passed on so that leuart_open can begin the leuart peripheral for transmission
 *
 * @param[in] tx_event,rx_event not used in this case
 *
 ******************************************************************************/

void ble_open(){
  //uint32_t tx_event, uint32_t rx_event are params removed
  LEUART_OPEN_STRUCT local_open_struct;

  local_open_struct.baudrate = HM10_BAUDRATE;
  local_open_struct.databits = HM10_DATABITS;
  local_open_struct.enable = HM10_ENABLE;
  local_open_struct.parity = HM10_PARITY;
  local_open_struct.rx_done_evt = 0;
  local_open_struct.rx_en = true;
  local_open_struct.rx_loc = LEUART0_RX_ROUTE;
  local_open_struct.rx_pin_en = true;
  local_open_struct.stopbits = HM10_STOPBITS;
  local_open_struct.tx_done_evt = 0;
  local_open_struct.tx_en = true;
  local_open_struct.tx_loc = LEUART0_TX_ROUTE;
  local_open_struct.tx_pin_en = true;

  leuart_open(LEUART0,&local_open_struct);
...
/***************************************************************************//**
 * @brief
 *   Calls leuart_start to start transmission of a string
 *
 * @param[in] string
...
bool ble_test(char *mod_name){
	...
	// ANSWER: Polling is continuous checking of a condition while interrupts only activate
	//         when some condition is raised, for example a peripheral sending a signal
	// How does interrupts benefit the system for low energy operation?
	// ANSWER: Interrupts only trigger on a signal, meaning a device can save
	//         power by not continuously checking for a condition which keeps the
	//         device on. With interrupts the device can go into sleep mode and conserve
	//         power until it needs to act
	// How does interrupts benefit the system that has multiple tasks?
	// ANSWER: The CPU can work on those other tasks and only stops when a peripheral
	//         raises the interrupt and gets handled
	...
	// Answer:  It could be stuck in the while loop if the receiver status is not set
  //          to true, it's a check to see if it's ready to receive data.
	//          For me peronally, it was stuck in the while loop because the routing was
  //          was incorrect which is another possibility
...
//	leuart_cmd_write(HM10_LEUART0, (LEUART_CMD_CLEARRX | LEUART_CMD_CLEARTX));
	...
	// DSD HM10 module?
	// ANSWER: You want to make sure you disconnect the module from any
	//         existing connections before you change the name
