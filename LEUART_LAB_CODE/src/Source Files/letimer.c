 ...
 *  February 13, 2022
 * @brief
 *  Defining and initializing functions dealing with running the LETIMER
 */
...
void letimer_pwm_open(LETIMER_TypeDef *letimer, APP_LETIMER_PWM_TypeDef *app_letimer_struct){
  ...
  // ZZZ are values that you must specify for this PWM specific driver from the online HAL documentation
	...
	//Stall clock until initialization is done
	while(letimer->SYNCBUSY);
   ...
   */
	...
	//I'm assuming compareset is the function that loads the values period_cnt, and period_active_cnt
	//So I just need to calculate period_cnt and period_active_cnt
	//The period_cnt should just be the the period in our app struct multiplied by the frequency
	period_cnt = app_letimer_struct->period * LETIMER_HZ;
	//Active period is the same thing, but using the active period in the struct
   ...
   * REP0 and REP1 bits
   */
	letimer->REP0 |= LETIMER_REP3;
	letimer->REP1 |= LETIMER_REP3;
  ...
  letimer->IFC = (app_letimer_struct->uf_irq_enable*app_letimer_struct->uf_cb) | (app_letimer_struct->comp1_irq_enable*app_letimer_struct->comp1_cb);
  letimer->IFC |= (app_letimer_struct->comp0_irq_enable*app_letimer_struct->comp0_cb);
	letimer->IEN = (app_letimer_struct->uf_irq_enable*app_letimer_struct->uf_cb) | (app_letimer_struct->comp1_irq_enable*app_letimer_struct->comp1_cb);
	letimer->IEN |= (app_letimer_struct->comp0_irq_enable*app_letimer_struct->comp0_cb);
...
/***************************************************************************//**
 * @brief
 *   The IRQ handler is essentially the interrupt service routine that
 *   is called when an interrupt is caught. Each potential interrupt
 *   has a defined way of being handled
 *
 * @details
 *   If statements are used to check if a specific interrupt flag has been raised for
 *   and whether it's aliged within the Comp 0, Comp1, and Uf flags specifically. Inside those if statements we
 *   handle the interrupts by adding scheduled event callbacks based on the flag raised.
 *   The Comp0 call back is not used in this lab.
 *
 * @note
 *  The interrupt flag is defined within the function as anding of the IF and IEN register in the letimer
 *  We then set the IFC register equal to this interrupt flag.
 *
 * @param[in] None
 *
 * @return None
